...
describes what users get. Pick a format with `-o`, or let usgs choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
usgs <command> -o table   # aligned columns for reading
usgs <command> -o jsonl   # one JSON object per line, for piping
usgs <command> -o json    # a single JSON array
usgs <command> -o csv     # spreadsheet friendly
usgs <command> -o tsv     # tab-separated
usgs <command> -o url     # just the URL column
usgs <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
usgs <command> --fields id,title,url
```
...
```bash
usgs <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
usgs <command>            # a table, because this is a terminal
usgs <command> | wc -l    # JSONL, because this is a pipe
```
